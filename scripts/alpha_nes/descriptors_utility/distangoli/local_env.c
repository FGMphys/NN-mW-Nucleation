...
int main(int argc,char *argv[])
{
	...
	if (argc!=9)
	{
		printf("%s [input file] [code or code file] [range] [nn input size] [range angolare] [nn angular size] [output suffix] [Number of Neighbours]\n",argv[0]);
		exit(1);
	}
 ...
 */
 ...
 ////TUTTI I DESCRITTORI///////
 //////////////////////////////
  int numaux=Nv*(Nv-1)/2; 
  sprintf(name_alldes,"alldes%s",suffix);
  sprintf(name_3Denv,"3Dlocalenv%s",suffix);
  FILE *file_alldes=fopen(name_alldes,"w");
//  FILE *file_3Denv=fopen(name_3Denv,"w");
 ///Intestazione
/*  fprintf(file_alldes,"\"dummy\"");
  fprintf(file_3Denv,"\"dummy\"");
  for (int g=0; g<Nv;g++){
      fprintf(file_alldes,",\"Des_R%d\"",g);
   }
  for (int g=0; g<Nv;g++){
      fprintf(file_3Denv,",\"ds_x%d\"",g);
      fprintf(file_3Denv,",\"ds_y%d\"",g);
      fprintf(file_3Denv,",\"ds_z%d\"",g);
   }
  for (int g=0; g<numaux;g++){
  ...
  *///scrittura descrittori
  ...
  //  fprintf(file_3Denv,"%d",code[i]);
    for (li=0;li<l;li++)
    {
      double v=SQR(1.-(ds[i][li].dist/range));
      int index=ds[i][li].index;
      fprintf(file_alldes,",%14.11g",v);
    //  fprintf(file_3Denv,",%14.11g",ds[i][li].dx);
     // fprintf(file_3Denv,",%14.11g",ds[i][li].dy);
      //fprintf(file_3Denv,",%14.11g",ds[i][li].dz);
    }
    //fprintf(file_3Denv,"\n");
