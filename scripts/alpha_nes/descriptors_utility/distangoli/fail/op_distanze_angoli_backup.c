...
int main(int argc,char *argv[])
{
  ...
  for (i=0;i<num;i++)
  {
    ...
    for (j=0;j<ds_num[i];j++)
    {
      ...
      int f=0;
      while ((f<ds_num[neighbour]) && (ds[neighbour][f].index!=i))
        f++;

      fprintf(file_interazioni,"%d %d",neighbour,f);

      assert(f<ds_num[neighbour]);
    }
    fprintf(file_interazioni,"\n");
  }
  ...
  {
    int j;
    for (j=0;j<num_angint[i];j++)
    {
      fprintf(file_interazioni_angolari,"%d %d %d ",angint_central[i][j],angint_selfindex[i][j],angint_jk[i][j]);
    }
  ...
  sprintf(name_descrittori,"descrittori%s",suffix);
  FILE *file_descrittori=fopen(name_descrittori,"w");
  ...
  for (i=0;i<num;i++)
  {
    ...
    fprintf(file_descrittori,"%d",code[i]);

    for (li=0;li<l;li++)
    {
      double v=SQR(1.-(ds[i][li].dist/range));
      int index=ds[i][li].index;
      fprintf(file_descrittori,",%g",v);
    }
    while (li++<expected)
      fprintf(file_descrittori,",0.");

    fprintf(file_descrittori,"\n");

  }

  fclose(file_descrittori);

  ////////////////////////////////////////////////
  ////    DESCRITTORI  ANGOLARI       ////////////
  ////////////////////////////////////////////////

  sprintf(name_descrittori_angolari,"descrittoriangolari%s",suffix);
  FILE *file_descrittori_angolari=fopen(name_descrittori_angolari,"w");



  // stampa descrittori angolari
  for (i=0;i<num;i++)
  {
    ...
    fprintf(file_descrittori_angolari,"%d",code[i]);

    for (li=0;li<l;li++)
    {
      fprintf(file_descrittori_angolari,",%g",da[i][li].angle);
    }
    while (li++<expected)
      fprintf(file_descrittori_angolari,",0.");

    fprintf(file_descrittori_angolari,"\n");
  }


  fclose(file_descrittori_angolari);
  ...
  for (i=0;i<num;i++)
  {
    ...
    for (li=0;li<l;li++)
    {
      fprintf(file_derivate_angolari,",%g,%g",da[i][li].dxij,da[i][li].dxik);
    }
    while (li++<expected)
    ...
    for (li=0;li<l;li++)
    {
      fprintf(file_derivate_angolari,",%g,%g",da[i][li].dyij,da[i][li].dyik);
    }
    while (li++<expected)
      fprintf(file_derivate_angolari,",0.,0.");
    ...
    for (li=0;li<l;li++)
    {
      fprintf(file_derivate_angolari,",%g,%g",da[i][li].dzij,da[i][li].dzik);
    }
    while (li++<expected)
      fprintf(file_derivate_angolari,",0.,0.");
