...
int compare_interdis(const void * a, const void * b)
    {

        if ( ((distangle*)a)->distjk <  ((distangle*)b)->distjk )
    return -1;
  else
    return 1;
  ...
  for (i=0;i<num;i++)
  {
    ...
    for (j=0;j<ds_num_angular[i]-1;j++)
    {
      for (k=j+1;k<ds_num_angular[i];k++)
      {
        ...
        double cutoffjk=SQR(1.-(distjk/2*range_angolare));

        da_ijk.distjk=distjk;
        da_ijk.angle=cutoffj*cutoffk*cutoffjk;
        ...
        double dcjk=(distjk-2*range_angolare)/(range_angolare*2*range_angolare*distjk);
        double dcxjk=dcjk*dist.x;
        double dcyjk=dcjk*dist.y;
        double dczjk=dcjk*dist.z;


        da_ijk.dxij=dcxij*cutoffk*cutoffjk-cutoffj*cutoffk*dcxjk;
        da_ijk.dyij=dcyij*cutoffk*cutoffjk-cutoffj*cutoffk*dcyjk;
        da_ijk.dzij=dczij*cutoffk*cutoffjk-cutoffj*cutoffk*dczjk;

        da_ijk.dxik=cutoffj*dcxik*cutoffjk+cutoffj*cutoffk*dcxjk;
        da_ijk.dyik=cutoffj*dcyik*cutoffjk+cutoffj*cutoffk*dcyjk;
        da_ijk.dzik=cutoffj*dczik*cutoffjk+cutoffj*cutoffk*dczjk;
