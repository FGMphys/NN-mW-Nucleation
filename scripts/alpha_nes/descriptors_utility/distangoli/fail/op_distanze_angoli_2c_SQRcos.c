  ...
  for (i=0;i<num;i++)
  {
    ...
    for (j=0;j<ds_num_angular[i]-1;j++)
    {
      for (k=j+1;k<ds_num_angular[i];k++)
      {
        ...
        da_ijk.angle=SQR(angle)*cutoffj*cutoffk;
        ...
        double danglexij=2*angle*(dij2*xik - xij*(xij*xik + yij*yik + zij*zik))/(Power(dij2,1.5)*Sqrt(dik2));
        double dangleyij=2*angle*(dij2*yik - yij*(xij*xik + yij*yik + zij*zik))/(Power(dij2,1.5)*Sqrt(dik2));
        double danglezij=2*angle*(dij2*zik - zij*(xij*xik + yij*yik + zij*zik))/(Power(dij2,1.5)*Sqrt(dik2));

        double danglexik=2*angle*(dik2*xij - xik*(xij*xik + yij*yik + zij*zik))/(Sqrt(dij2)*Power(dik2,1.5));
        double dangleyik=2*angle*(dik2*yij - yik*(xij*xik + yij*yik + zij*zik))/(Sqrt(dij2)*Power(dik2,1.5));
        double danglezik=2*angle*(dik2*zij - zik*(xij*xik + yij*yik + zij*zik))/(Sqrt(dij2)*Power(dik2,1.5));

        da_ijk.dxij=danglexij*cutoffj*cutoffk+SQR(angle)*dcxij*cutoffk;
        da_ijk.dyij=dangleyij*cutoffj*cutoffk+SQR(angle)*dcyij*cutoffk;
        da_ijk.dzij=danglezij*cutoffj*cutoffk+SQR(angle)*dczij*cutoffk;

        da_ijk.dxik=danglexik*cutoffj*cutoffk+SQR(angle)*cutoffj*dcxik;
        da_ijk.dyik=dangleyik*cutoffj*cutoffk+SQR(angle)*cutoffj*dcyik;
        da_ijk.dzik=danglezik*cutoffj*cutoffk+SQR(angle)*cutoffj*dczik;
