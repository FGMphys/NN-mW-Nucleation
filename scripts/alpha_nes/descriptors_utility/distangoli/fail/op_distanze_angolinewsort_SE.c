...
int main(int argc,char *argv[])
{
  ...
  for (i=0;i<num;i++)
  {
    ...
    if (ds_num_angular[i]<n_input_angolare)
    {
	printf("Warning: numero di vicini angolari insufficiente\n");
    }
    else
	{
		ds_num_angular[i]=n_input_angolare;
		//da_num[i]=((n_input_angolare-1)*(n_input_angolare-2))/2;
	}

    distangle da_ijk,buffer;
    vector olddist;

    for (j=0;j<ds_num_angular[i]-2;j++)
    {
      printf("%d  ",ds_num_angular[i]);
      for (k=j+1;k<ds_num_angular[i]-1;k++)
      {
	...
	if ((cj1!=k) && (ck2!=j))
	{
        da_ijk.angle*=SQR(ds[i][j].dist-ds[i][cj1].dist);
        printf(" %d %d ",j,cj1);
        da_ijk.angle*=SQR(ds[i][k].dist-ds[i][ck2].dist);
        printf(" %d %d ",k,ck2);
	}
        if (cj2>=0){
            da_ijk.angle*=SQR(ds[i][j].dist-ds[i][cj2].dist);
            printf(" %d %d ",j,cj2);}
        if (ck1<ds_num_angular[i]){
           da_ijk.angle*=SQR(ds[i][k].dist-ds[i][ck1].dist);
           printf(" %d %d, ",k,ck1);
        }
        else
            {
            da_ijk.angle*=SQR(ds[i][k].dist-range_angolare);
            printf(" %d %g, ",k,range_angolare);
        }
        ...
        /*if (da_num[i]==((n_input_angolare-1)*(n_input_angolare-2))/2)
          {
            k=ds_num_angular[i]-1;
            j=ds_num_angular[i]-2;
         }*/
 ...
 //////////////////////////////
  ...
  for (int g=0; g<((n_input_angolare-1)*(n_input_angolare-2))/2;g++){
  ...
  for (i=0;i<num;i++)
  {
    ...
    expected=((n_input_angolare-1)*(n_input_angolare-2))/2;;
    l=da_num[i];
    printf("Ecco danumi %d",da_num[i]);
    li=0;
