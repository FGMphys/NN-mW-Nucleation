...
int main(int argc,char *argv[])
{
	...
	if (argc!=9)
	{
		printf("%s [input file] [code or code file] [range] [nn input size] [range angolare] [nn angular size] [output suffix] [maxangular neigh] \n",argv[0]);
		exit(1);
	}
  ...
  int max_angularneigh=atoi(argv[8]);
  char suffix[100];
 ...
 //////////////////////////////
  ...
  for (i=0;i<num;i++)
  {
    ...
    expected=max_angularneigh;
    int expected2=n_input_angolare;
    l=da_num[i];
    li=0;
    int aux=0;
    int check=0;
    ...
    for (li=0;li<ds_num_angular[i]-1;li++)
    {
     for (h=li+1;h<ds_num_angular[i];h++){
      fprintf(file_alldes,",%14.11g",da[i][aux].angle);
      aux++;
      check++;
    }
   while (h++<expected){
    ...
    int u=0;
    while (li++<expected-1){
      for  (int u=li+1;u<expected;u++){
      fprintf(file_alldes,",0.");
      check++;
    }
    }
   
    while (check++<expected2){
      fprintf(file_alldes,",0.");
    }
    fprintf(file_alldes,"\n");
  }
