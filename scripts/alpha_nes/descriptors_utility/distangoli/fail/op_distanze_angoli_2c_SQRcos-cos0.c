  ...
  for (i=0;i<num;i++)
  {
    ...
    for (j=0;j<ds_num_angular[i]-1;j++)
    {
      for (k=j+1;k<ds_num_angular[i];k++)
      {
        ...
        da_ijk.angle=SQR(angle-1./3.)*cutoffj*cutoffk;
        ...
        double danglexij=2*(angle-1./3.)*(dij2*xik - xij*(xij*xik + yij*yik + zij*zik))/(Power(dij2,1.5)*Sqrt(dik2));
        double dangleyij=2*(angle-1./3.)*(dij2*yik - yij*(xij*xik + yij*yik + zij*zik))/(Power(dij2,1.5)*Sqrt(dik2));
        double danglezij=2*(angle-1./3.)*(dij2*zik - zij*(xij*xik + yij*yik + zij*zik))/(Power(dij2,1.5)*Sqrt(dik2));

        double danglexik=2*(angle-1./3.)*(dik2*xij - xik*(xij*xik + yij*yik + zij*zik))/(Sqrt(dij2)*Power(dik2,1.5));
        double dangleyik=2*(angle-1./3.)*(dik2*yij - yik*(xij*xik + yij*yik + zij*zik))/(Sqrt(dij2)*Power(dik2,1.5));
        double danglezik=2*(angle-1./3.)*(dik2*zij - zik*(xij*xik + yij*yik + zij*zik))/(Sqrt(dij2)*Power(dik2,1.5));

        da_ijk.dxij=danglexij*cutoffj*cutoffk+SQR(angle-1./3.)*dcxij*cutoffk;
        da_ijk.dyij=dangleyij*cutoffj*cutoffk+SQR(angle-1./3.)*dcyij*cutoffk;
        da_ijk.dzij=danglezij*cutoffj*cutoffk+SQR(angle-1./3.)*dczij*cutoffk;

        da_ijk.dxik=danglexik*cutoffj*cutoffk+SQR(angle-1./3.)*cutoffj*dcxik;
        da_ijk.dyik=dangleyik*cutoffj*cutoffk+SQR(angle-1./3.)*cutoffj*dcyik;
        da_ijk.dzik=danglezik*cutoffj*cutoffk+SQR(angle-1./3.)*cutoffj*dczik;
