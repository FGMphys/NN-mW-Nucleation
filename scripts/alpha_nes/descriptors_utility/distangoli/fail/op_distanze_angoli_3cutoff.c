...
int main(int argc,char *argv[])
{
  ...
  for (i=0;i<num;i++)
  {
    ...
    while ((im_index<im->howmany[i]) && (ds_num[i]<n_input))
    {

      //assert(im_index<im->howmany[i]);
  ...
  for (i=0;i<num;i++)
  {
    ...
    for (j=0;j<ds_num_angular[i]-1;j++)
    {
      for (k=j+1;k<ds_num_angular[i];k++)
      {
        ...
        // calcolo distanza jk
        olddist.x=pos[da_ijk.indexj].x-pos[da_ijk.indexk].x;
        olddist.y=pos[da_ijk.indexj].y-pos[da_ijk.indexk].y;
        olddist.z=pos[da_ijk.indexj].z-pos[da_ijk.indexk].z;
        ...
        double distjk=sqrt(SQR(dist.x)+SQR(dist.y)+SQR(dist.z));
        double cutoffjk=SQR(1.-(distjk/range_angolare));


        da_ijk.angle=0.5*(angle+1)*cutoffj*cutoffk*cutoffjk;
        ...
        double dcjk=2*(distjk-range_angolare)/(range_angolare*range_angolare*distjk);
        double dcxjk=dcjk*dist.x;
        double dcyjk=dcjk*dist.y;
        double dczjk=dcjk*dist.z;
        ...
        da_ijk.dxij=danglexij*cutoffj*cutoffk*cutoffjk+0.5*(angle+1)*dcxij*cutoffk*cutoffjk-0.5*(angle+1)*cutoffj*cutoffk*dcxjk;
        da_ijk.dyij=dangleyij*cutoffj*cutoffk*cutoffjk+0.5*(angle+1)*dcyij*cutoffk*cutoffjk-0.5*(angle+1)*cutoffj*cutoffk*dcyjk;
        da_ijk.dzij=danglezij*cutoffj*cutoffk*cutoffjk+0.5*(angle+1)*dczij*cutoffk*cutoffjk-0.5*(angle+1)*cutoffj*cutoffk*dczjk;

        da_ijk.dxik=danglexik*cutoffj*cutoffk*cutoffjk+0.5*(angle+1)*cutoffj*dcxik*cutoffjk+0.5*(angle+1)*cutoffj*cutoffk*dcxjk;
        da_ijk.dyik=dangleyik*cutoffj*cutoffk*cutoffjk+0.5*(angle+1)*cutoffj*dcyik*cutoffjk+0.5*(angle+1)*cutoffj*cutoffk*dcyjk;
        da_ijk.dzik=danglezik*cutoffj*cutoffk*cutoffjk+0.5*(angle+1)*cutoffj*dczik*cutoffjk+0.5*(angle+1)*cutoffj*cutoffk*dczjk;
