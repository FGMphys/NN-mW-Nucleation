...
int insertionSortBlocchi(void *array,void *element,void *buffer,int *length,size_t size,int (*compar)(const void*,const void*),int limit)
{
        int l=*length;

  memcpy(array+l*size,element,size);

        while ((l>limit) && (compar(array+(l-1)*size,array+l*size)==1))
        {
    memcpy(buffer,array+l*size,size);
    memcpy(array+l*size,array+(l-1)*size,size);
    memcpy(array+(l-1)*size,buffer,size);
...
int main(int argc,char *argv[])
{
	...
	if (argc!=9)
	{
		printf("%s [input file] [code or code file] [range] [nn input size] [range angolare] [nn angular size] [output suffix] [limit]\n",argv[1]);
		exit(1);
	}
  ...
  for (i=0;i<num;i++)
  {
    ...
    while (im_index<im->howmany[i]) 
    {
      ...
      if (ds_num[i]>n_input)
         ds_num[i]=n_input;
      else if (dij.dist<range_angolare)
          ds_num_angular[i]++;
  ...
  for (i=0;i<num;i++)
  {
	...
	int limit=0;
        int ins=insertionSortBlocchi(da[i],&da_ijk,&buffer,da_num+i,sizeof(distangle),compare_distangle,limit);
    ...
    for (j=4;j<ds_num_angular[i]-1;j++)
    {
	...
	int limit=6+4*(ds_num_angular[i]-4);
        int ins=insertionSortBlocchi(da[i],&da_ijk,&buffer,da_num+i,sizeof(distangle),compare_distangle,limit);
